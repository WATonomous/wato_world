...
# Build-time provenance, baked into the image as environment variables and read
# back at runtime by wato_common.provenance.
#
# These MUST be baked in rather than discovered at runtime: a deploy image has
# no .git directory, so `git rev-parse` inside the container silently returns
# "" and every artifact ends up claiming an unknown source revision. The values
# are supplied by modules/.env (generated by watod_scripts/watod-setup-env.sh).
#
# Declared last, after the expensive layers, so that a new commit invalidates
# only this cheap layer and not the dependency install above.
ARG WATO_GIT_COMMIT=""
ARG WATO_GIT_DIRTY="false"
ARG WATO_BUILD_TIME=""
ENV WATO_GIT_COMMIT=${WATO_GIT_COMMIT} \
    WATO_GIT_DIRTY=${WATO_GIT_DIRTY} \
    WATO_BUILD_TIME=${WATO_BUILD_TIME}
# NOTE: WATO_BASE_IMAGE is deliberately NOT set here. Each component's
# dependencies stage sets it from its own digest-pinned BASE_IMAGE arg, and this
# stage builds FROM that — re-declaring it here would overwrite the real digest
